...
func (e *ToJsonEngine) createWorkerQueued(in chan gopacket.Packet, out chan error, ready ReadyNotifier, file_fd *os.File) {
	go func() {
		for {
			ready.WorkerReady(in)
			packet := <-in
			err := writeJson.WritePacketjson(packet, file_fd)
			out <- err
		}
	}()
}
